	...
	$.fn.instantVideoPlayer = function(options){
		...
		return this.each(function(i){
			...
			var html5Video	 = "<video id='_video"+i+"'";
				...
				if(!options.defaultControls && $("video" , videoContainer)[0].canPlayType){
			...
			if(options.timer){
				...
				function updateTimerUI(){
					if(videoPlayer.currentTime < 9){
						fillerTime = "00:0";	
					} else if(videoPlayer.currentTime > 9){
						fillerTime = "00:";	
					}
					timer.html(fillerTime+Math.round(videoPlayer.currentTime));
				}
			}
